...
function GlobalFilter({ filter, setFilter }) {
    ...
    return (
        <fieldset className="w-full space-y-1 text-coolGray-800 sm:my-10">
            ...
            <div className="relative">
                ...
                <input type="search" name="Search" placeholder="Search..." className="w-32 py-2 pl-10 text-sm rounded-full sm:w-auto focus:outline-none bg-coolGray-100 text-coolGray-800 focus:bg-coolGray-50 focus:border-violet-600" value={value || ''} onChange={handleChange} />
            </div>
        </fieldset>
...
function Pagination({ pages, index, next, prev, canNext, canPrev, count, results, gotoPage, size, setSize }) {
    ...
    const renderTag = () => {
        ...
        return pages.map(pageIndex => {
            ...
            const isActive = pageIndex === index ? "z-10 bg-indigo-50 border-indigo-500 text-indigo-600 relative inline-flex items-center px-4 py-2 border text-sm font-medium" : "bg-white border-gray-300 text-gray-500 hover:bg-gray-50 relative inline-flex items-center px-4 py-2 border text-sm font-medium";
                    ...
                    <div>
                        ...
                        >
                            {DEFAULT_PAGE_SIZE.map(pageSize => (
                                <option key={pageSize} value={pageSize} className="text-gray-700 block px-4 py-2 text-sm">
                                    Show {pageSize}
                                </option>
                            ))}
                        </select>
